...
static mp_obj_t py_sensor_snapshot_finish(uint n_args, const mp_obj_t *args, mp_map_t *kw_args)
{
    bool auto_restart = py_helper_keyword_int(n_args, args, 0, kw_args, MP_OBJ_NEW_QSTR(MP_QSTR_auto_restart), false);

    mp_obj_t image = py_image(0, 0, 0, 0);
    // Note: OV2640 JPEG mode can __fatal_error().
    int ret = sensor.snapshot_finish(&sensor, (image_t *) py_image_cobj(image), auto_restart);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_KW(py_sensor_snapshot_finish_obj, 0, py_sensor_snapshot_finish);
